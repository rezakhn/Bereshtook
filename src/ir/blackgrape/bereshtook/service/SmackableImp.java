...
public class SmackableImp implements Smackable {
	final static private String TAG = "Bereshtook.SmackableImp";
	...
	static final DiscoverInfo.Identity YAXIM_IDENTITY = new DiscoverInfo.Identity("client",
					BereshtookApplication.XMPP_IDENTITY_NAME,
					BereshtookApplication.XMPP_IDENTITY_TYPE);
	...
	private final BereshtookConfiguration mConfig;
	...
	private static final String PING_ALARM = "ir.blackgrape.bereshtook.PING_ALARM";
	private static final String PONG_TIMEOUT_ALARM = "ir.blackgrape.bereshtook.PONG_TIMEOUT_ALARM";
	...
	public boolean doConnect(boolean create_account) throws BereshtookXMPPException {
	...
	private void initServiceDiscovery() {
		...
		String app_name = mService.getString(ir.blackgrape.bereshtook.R.string.app_name);
		String version_name = mService.getString(ir.blackgrape.bereshtook.R.string.version_name);
	...
	public void addRosterItem(String user, String alias, String group)
			throws BereshtookXMPPException {
		tryToAddRosterEntry(user, alias, group);
	}

	public void removeRosterItem(String user) throws BereshtookXMPPException {
	...
	public void renameRosterItem(String user, String newName)
		...
		if (!(newName.length() > 0) || (rosterEntry == null)) {
			throw new BereshtookXMPPException("JabberID to rename is invalid!");
		}
	...
	private void tryToConnect(boolean create_account) throws BereshtookXMPPException {
		try {
	...
	private void tryToMoveRosterEntryToGroup(String userName, String groupName)
		...
		if (groupName.length() == 0)
			...
			try {
				...
				throw new BereshtookXMPPException("tryToMoveRosterEntryToGroup", e);
			}
		}
	}
	...
	private void removeRosterEntryFromGroups(RosterEntry rosterEntry)
		...
		for (RosterGroup group : oldGroups) {
			if(!group.getName().equals("همه برشتوک ها"))
				tryToRemoveUserFromGroup(group, rosterEntry);
		}
	}

	private void tryToRemoveUserFromGroup(RosterGroup group,
			RosterEntry rosterEntry) throws BereshtookXMPPException {
		try {
			group.removeEntry(rosterEntry);
		} catch (XMPPException e) {
			throw new BereshtookXMPPException("tryToRemoveUserFromGroup", e);
		}
	}

	private void tryToRemoveRosterEntry(String user) throws BereshtookXMPPException {
		try {
			RosterEntry rosterEntry = mRoster.getEntry(user);
			...
			throw new BereshtookXMPPException("tryToRemoveRosterEntry", e);
		}
	}
	...
	private void tryToAddRosterEntry(String user, String alias, String group)
		...
		try {
			...
			throw new BereshtookXMPPException("tryToAddRosterEntry", e);
		}
	}
